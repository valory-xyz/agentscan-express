...
export const discord_allowed_channels = pgTable(
...
export const component_processing_status = pgTable(
  "component_processing_status",
  {
  ...
  (table) => [
    index("idx_component_status").using(
      "btree",
...
export const contract_abis = pgTable(
  "contract_abis",
  {
  ...
  (table) => [
    unique("contract_abis_address_chain_id_key").on(
      table.address,
      table.chain_id
    ),
  ]
...
export const metadata_embeddings = pgTable(
  "metadata_embeddings",
  {
  ...
  (table) => [
    index("idx_metadata_embeddings_agent_id").using(
      "btree",
      table.agent_id.asc().nullsLast().op("text_ops")
    ),
    index("idx_metadata_embeddings_component_id").using(
      "btree",
      table.component_id.asc().nullsLast().op("text_ops")
    ),
    index("idx_metadata_embeddings_hnsw")
      .using("hnsw", table.embedding.asc().nullsLast().op("vector_cosine_ops"))
      .with({ m: "16", ef_construction: "200" }),
    index("idx_metadata_embeddings_service_id").using(
      "btree",
      table.service_id.asc().nullsLast().op("text_ops")
    ),
    index("idx_metadata_id").using(
      "btree",
      table.id.asc().nullsLast().op("text_ops")
    ),
    check(
      "one_id_required",
      sql`((((service_id IS NOT NULL))::integer + ((agent_id IS NOT NULL))::integer) + ((component_id IS NOT NULL))::integer) >= 1`
    ),
  ]
);
...
export const telegram_allowed_supergroups = pgTable(
  "telegram_allowed_supergroups",
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    chat_id: bigint({ mode: "number" }).notNull(),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    enabled_by: bigint({ mode: "number" }).notNull(),
    enabled_at: timestamp({ withTimezone: true, mode: "string" }).default(
      sql`CURRENT_TIMESTAMP`
    ),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
  ...
  (table) => [unique("unique_chat_thread").on(table.chat_id, table.thread_id)]
);
...
export const users = pgTable(
  ...
  (table) => [
    index("idx_users_privy_did").using(
      "btree",
      table.privy_did.asc().nullsLast().op("text_ops")
    ),
...
export const context_labels = pgTable(
  "context_labels",
  {
  ...
  (table) => [
    index("idx_context_labels_context_id").using(
      "btree",
      table.context_id.asc().nullsLast().op("text_ops")
    ),
    index("idx_context_labels_label").using(
      "btree",
...
export const processing_contracts = pgTable(
  "processing_contracts",
  {
    address: text().notNull(),
    chain_id: integer().notNull(),
    started_at: timestamp({ withTimezone: true, mode: "string" }).default(
      sql`CURRENT_TIMESTAMP`
    ),
  },
  (table) => [
    index("idx_processing_contracts_address").using(
      "btree",
...
export const code_embeddings = pgTable(
  "code_embeddings",
  {
  ...
  (table) => [
    index("idx_code_embeddings_component_id").using(
      "btree",
      table.component_id.asc().nullsLast().op("text_ops")
    ),
    index("idx_code_embeddings_hnsw_embedding")
      .using("hnsw", table.embedding.asc().nullsLast().op("vector_cosine_ops"))
      .with({ m: "16", ef_construction: "200" }),
    primaryKey({
      columns: [table.component_id, table.file_path],
      name: "code_embeddings_pkey",
    }),
  ]
);

export const context_processing_status = pgTable(
...
export const context_embeddings = pgTable(
  "context_embeddings",
  {
    id: text().notNull(),
  ...
  (table) => [
    index("context_embeddings_embedding_idx").using(
      "hnsw",
      table.embedding.asc().nullsLast().op("vector_cosine_ops")
    ),
    index("context_embeddings_embedding_idx1").using(
      "hnsw",
      table.embedding.asc().nullsLast().op("vector_cosine_ops")
    ),
    index("context_embeddings_name_idx").using(
      "gin",
      sql`to_tsvector('english'::regconfig`
    ),
    index("idx_context_embeddings_active")
      .using("btree", table.deleted_at.asc().nullsLast().op("timestamptz_ops"))
      .where(sql`(deleted_at IS NULL)`),
    index("idx_context_embeddings_company").using(
      "btree",
      table.team_name.asc().nullsLast().op("text_ops")
    ),
    index("idx_context_embeddings_company_embedding").using(
      "btree",
      table.team_name.asc().nullsLast().op("text_ops"),
      table.embedding.asc().nullsLast().op("text_ops")
    ),
    index("idx_context_embeddings_hnsw_embedding")
      .using("hnsw", table.embedding.asc().nullsLast().op("vector_cosine_ops"))
      .with({ m: "16", ef_construction: "200" }),
    index("idx_context_embeddings_id").using(
      "btree",
      table.id.asc().nullsLast().op("text_ops")
    ),
    index("idx_context_embeddings_location").using(
      "btree",
      table.location.asc().nullsLast().op("text_ops")
    ),
    index("idx_context_embeddings_name").using(
      "btree",
