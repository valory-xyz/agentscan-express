import { pgTable, uuid, text, timestamp, index, unique, integer, bigint, boolean, foreignKey, jsonb, vector, check, primaryKey, pgEnum } from "drizzle-orm/pg-core"
...
export const context_embeddings = pgTable("context_embeddings", {
	...
	index("idx_context_embeddings_company").using("btree", table.company_id.asc().nullsLast().op("text_ops"), table.type.asc().nullsLast().op("text_ops")),
	index("idx_context_embeddings_company_embedding").using("btree", table.company_id.asc().nullsLast().op("text_ops"), table.embedding.asc().nullsLast().op("text_ops")),
	...
	primaryKey({ columns: [table.id, table.type, table.location], name: "context_embeddings_pkey"}),
]);
